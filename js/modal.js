...
class ModalFenster {
  ...
  constructor() {
    for (let i = 0; i < this.m_buttons.length; i++) {
      ...
      this.m_buttons[i].addEventListener("click", () => {
        //todo делегировать один листенер (modal_controls) на все кнопки
        this.close();
        this.m_callbacks[i]();
      });
    }
    this.m_window.addEventListener("keydown", (e) => {
      if (e.code === "Enter" || e.code === "NumpadEnter") {
        if (!this.m_ctrl)
          //если нет блока кнопок - закрыть на ENTER
          this.close();
      }
  ...
  open(title, body, buttons) {
    ...
    if (buttons) {
      ...
      for (let i = buttons.length; i < this.m_buttons.length; i++) {
        //на всяк случай пройти до конца и убрать лишние кнопки
  ...
  close() {
    if (this.m_window.style.display == "none") return; //костыль - если окно не открыто не обрабатывать клик (иначе закрывается шторка)
    this.m_open = false;
